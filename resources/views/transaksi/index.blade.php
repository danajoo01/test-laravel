...
<div class="row">
  ...
  <div class="col-lg-2">
    <a href="{{ route('transaksi.create') }}" class="btn btn-primary btn-rounded btn-fw"><i class="fa fa-plus"></i> Add Transaction</a>
  </div>
    <div class="col-lg-12">
                      ...
                      @foreach($datas as $data)
                        <tr>
                          ...
                          <form action="{{ route('transaksi.update', $data->id) }}" method="post" enctype="multipart/form-data">
                            {{ csrf_field() }}
                            {{ method_field('put') }}
                            <button class="dropdown-item" onclick="return confirm('You are sure this data has returned ?')"> already back
                            </button>
                          </form>
                          @endif
                        ...
                        <form action="{{ route('transaksi.update', $data->id) }}" method="post" enctype="multipart/form-data">
                            {{ csrf_field() }}
                            {{ method_field('put') }}
                            <button class="btn btn-info btn-xs" onclick="return confirm('You are sure this data has returned ?')">already back
                            </button>
                          </form>
                          @else
                          -
